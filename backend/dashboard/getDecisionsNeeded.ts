...
// Mirrors /context/linked-classic-app/lib/getDecisionsNeeded.sql.

type Params = { orgId: string | null }

export type DecisionNeededRow = {
  project_id: string
  project_name: string
...
export default async function getDecisionsNeeded(
  req: { params: Params; user: User },
): Promise<DecisionNeededRow[]> {
  const orgId = req.params.orgId ?? ''
  const result = await forgeSupabaseShadow2.query<DecisionNeededRow>(SQL, [orgId])
  return result.data
}
